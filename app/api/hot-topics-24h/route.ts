  ...
  try {
    ...
    if (!forceRefresh) {
      ...
      if (hotTopics.length > 0) {
        console.log('[API] 从数据库返回24小时热点话题')
    ...
    console.log('[API] 开始分析24小时热点话题...')
    const hotTopics = await analyzeHotTopicsV2(24)
    ...
    console.log(`[API] 成功分析并保存 ${hotTopics.length} 个24小时热点话题`)
    ...
    console.error('[API] 24小时热点分析失败:', error)
    return NextResponse.json({
      success: false,
